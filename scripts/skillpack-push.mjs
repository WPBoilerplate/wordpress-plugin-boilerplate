#!/usr/bin/env node
/**
 * WordPress Plugin Boilerplate — Skillpack Push
 *
 * Distributes skills from the canonical .ai/skills/ skillpack to
...
function main() {
	...
	if ( ! fs.existsSync( SKILLPACK_DIR ) ) {
		console.error( `❌  Skillpack not found at .ai/skills/` );
	...
	if ( skills.length === 0 ) {
		console.warn( '⚠️   No skills found in .ai/skills/ — nothing to push.' );
		process.exit( 0 );
	}
